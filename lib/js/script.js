...
$(function(){
    ...
    $.getJSON("https://api.moneypot.com/v1/list-bets?app_id="+config.app_id+"&access_token="+config.access_token, function(json){
        var data = json;
        for(var i=data.length-1; i>=0; i--){
            betsAmount++;
            
            var real_outcome = 0;
            for(var j=0; j<data[i].payouts.length; j++){
                if(data[i].raw_outcome < data[i].payouts[j].to && data[i].raw_outcome >= data[i].payouts[j].from){
                    real_outcome = j;
                    break;
                }
            }
            ...
            var win = parseFloat(data[i].profit) >= 0;
            
            cell1.innerHTML = "<a href=\"https://www.moneypot.com/bets/"+data[i].id+"\" target=\"blank\">"+data[i].id+"</a>";
            cell2.innerHTML = data[i].uname;
            cell3.innerHTML = (data[i].wager/100).formatMoney(2, '.', ',')+" Bits";
            cell4.innerHTML = multiplier+"x";
            cell5.innerHTML = (win?"+":"")+(data[i].profit/100).formatMoney(2, '.', ',');
    ...
    function render(){
        ...
        ctx.drawImage(images[0], game.width/2 - 320, game.height/2 - 235);
...
function createCoin(payload){
    ...
    var id = data.bet_id,
        ...
        username = data.uname,
        bet = parseFloat(data.wager/100).formatMoney(2, '.', ','),
...
$('#depositButton').click(function() {
    var windowUrl = 'https://www.moneypot.com/dialog/deposit?app_id=1320';
...
$('#withdrawButton').click(function() {
    var windowUrl = 'https://www.moneypot.com/dialog/withdraw?app_id=1320';
    var windowName = 'manage-auth';
    var windowOpts = 'width=420,height=350,left=100,top=100';
    var windowRef = window.open(windowUrl, windowName, windowOpts);
    windowRef.focus();
...
function onloadCallback() {
    grecaptcha.render('faucetClaimCaptcha', {
        'sitekey' : '6LepJBYTAAAAAGHwiGkt5OTb51waqulCNGNSAc2Z',
